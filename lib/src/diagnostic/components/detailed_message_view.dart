...
var DetailedMessageView = react.registerComponent(() => new _DetailedMessageView());
  ...
  render() {
    ...
    if (messages.length > 0) {
      ...
      messages.forEach((context) {
        messageList.add(react.li({}, Message({'context': context, 'detailed': true, 'onClose': onCloseMessage})));
      });
      body = react.ul({}, messageList);
    } else {
      body = react.p({'className': 'wsdp-detailed-message-view-empty'}, 'No messages. Click "+" on a message to view it here.');
    }

    return CollapsiblePanel({'title': 'Messages'},
      react.div({'className': 'wsdp-detailed-message-view'}, [
        body,
